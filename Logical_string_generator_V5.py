...
while True:
    try:
        ...
        if words_in_sentence == "r":
            ...
            break
        elif int(words_in_sentence) < 1 or int(words_in_sentence) > 20:
            raise ValueError
        words_in_sentence = int(words_in_sentence)
        break
    except:
        print("Enter an integer from 1 to 20 or type 'r'.")
...
while True:
    ...
    print("-----------------------------------------------------------------------------------------------------------")
    if spaces > 10 and len(string) > 104:
        print(f"""{string[:new_line]}
{string[new_line + 1:]}""")
    else:
        print(string)
    print("-----------------------------------------------------------------------------------------------------------")
    ...
    while True:
        try:
            ...
            print("Enter an integer from 1 to 20, type 'r', or type 'e'.")
            words_in_sentence = temp_words
...
"""
Original word lengths from source:

def word_length():
    possibility = random.randint(0, 235836)
    possibilities = (2, 162, 1582, 11812, 17084, 47073, 70942, 103345, 121051,
                     147064, 177942, 198404, 208169, 211546, 217471, 217669, 218097,
                     219910, 234849, 235691, 235773, 235814, 235831, 235836)
    lengths = (1, 2, 3, 5, 4, 8, 7, 9, 6, 11, 10, 12, 14, 16, 15, 20, 19, 17, 13, 18,
               21, 22, 23, 24)

    for vari in range(24):
        if possibility <= possibilities[vari]:
            return lengths[vari]
